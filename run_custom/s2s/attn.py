...
__all__ = [
    'DotProdScoring', 'LearnableScoring', 'causal_mask', 'Attention', 'SelfAttention', 'CrossAttention',
]
...
class DotProdScoring(nn.Module):
...
class LearnableScoring(nn.Module):
    def __init__(self, embed_dim, block_size, num_heads, scale, bias=True, dtype=None, device=None) -> None:
        ...
        self.Ws = nn.ModuleList( [nn.Bilinear(in1_features=self.head_dim, in2_features=self.head_dim, out_features=1, bias=bias, **self.factory) for _ in range(num_heads)] )
